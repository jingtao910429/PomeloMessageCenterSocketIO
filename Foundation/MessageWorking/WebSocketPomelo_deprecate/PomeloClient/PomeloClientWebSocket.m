...
#import "PomeloClientWebSocket.h"
...
@interface PomeloClientWebSocket (PrivateMethod)
/**
 *  发送消息
 *
 *  @param data 发送的数据
...
@implementation PomeloClientWebSocket
...
- (void)handleHandshake:(NSData *)theData{
    NSDictionary *data = [PomeloClientWebSocket decodeJSON:theData error:nil];
...
- (NSData *)encodeWithReqId:(NSInteger)reqId andRoute:(NSString *)route andMsg:(NSDictionary *)msg{
    ...
    if (_clientProtos && [_clientProtos objectForKey:route]) {
        ...
        NSString *str =[PomeloClientWebSocket encodeJSON:msg error:nil];
...
- (NSDictionary *)deCompose:(NSMutableDictionary *)msg{
    ...
    if (_serverProtos && [_serverProtos objectForKey:route]) {
        ...
        return [PomeloClientWebSocket decodeJSON:[msg objectForKey:@"body"] error:nil];
    }
    return msg;
}
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket{
    ...
    NSString *handStr = [PomeloClientWebSocket encodeJSON:handshakeDict error:nil];
